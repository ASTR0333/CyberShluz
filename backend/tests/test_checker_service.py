...
def test_inventory_uses_lms_and_reaches_wdc_through_it(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert set(inventory["all"]["children"]) == {"lms", "wdc"}
        lms = inventory["all"]["children"]["lms"]["hosts"]["l-ms"]
        assert lms["ansible_host"] == "203.0.113.10"
        assert "ansible_ssh_common_args" not in lms
