...
- `key-stand<ID>-student` — пользователь `student`, без `sudo` и групп `sudo`/`wheel`, используется web-terminal.

Дополнительно:

- root заблокирован;
- парольная и keyboard-interactive SSH-аутентификация отключена;
- студент может добавить личный публичный ключ, но его приватная часть не передаётся системе;
...
После этого RDP-клиент подключается к `localhost:13389`. Фактическая команда для
конкретного стенда показана на странице его статуса.

Обычный сценарий для студента не требует локального RDP-клиента и ручного
туннеля: кнопка «Открыть W-DC» на странице стенда создаёт короткоживущий
SSH-мост на стороне CyberShluz и сразу открывает W-DC в новой вкладке через
Apache Guacamole. Backend проверяет владельца стенда и доступность W-DC, а
параметры RDP передаются Guacamole в подписанном и зашифрованном JSON-блоке.
Для этого в `.env` должен находиться отдельный случайный 128-битный ключ:

```dotenv
GUACAMOLE_JSON_SECRET_KEY=<ровно 32 шестнадцатеричных символа>
```

`scripts/configure.sh` создаёт этот ключ автоматически, а `scripts/deploy.sh`
добавляет его в старый env-файл при первом обновлении. Ручная команда выше
остаётся резервным способом диагностики.
...
- L-MS доступна по SSH через floating IP;
- L-NFS и L-PGSQL доступны через L-MS как bastion;
...
### 11.2. Ручные подтверждения

Текущий API-доступ не позволяет надёжно доказать:

- появление `w-dc.cyberprotect.test` среди узлов хранения;
- создание RepoW и RepoL в веб-консоли.

Эти пункты подтверждаются отдельными чекбоксами. До подтверждения результат — `REVIEW_REQUIRED`, оценка в Moodle не отправляется. После результата `PASSED` стенд и отчёт сохраняются, пока пользователь явно не нажмёт «Завершить работу»; это не позволяет фоновой очистке стереть итог раньше, чем студент его увидит.

### 11.3. Исправления команд методички
