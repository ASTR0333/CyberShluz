...
туннеля: кнопка «Открыть W-DC» на странице стенда открывает штатную консоль
виртуальной машины в панели КИ. Backend проверяет владельца стенда и формирует
ссылку по сохранённому UUID инстанса W-DC. Адрес панели задаётся в `.env`:
