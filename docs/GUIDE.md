...
Текущий SSH/API-доступ не позволяет надёжно доказать:

- запуск Windows-служб Storage Node, Catalog Browser и Elasticsearch на W-DC;
- появление `w-dc.cyberprotect.test` среди узлов хранения;
- создание RepoW и RepoL в веб-консоли.

Эти пункты подтверждаются отдельными чекбоксами. До подтверждения результат — `REVIEW_REQUIRED`, оценка в Moodle не отправляется, автоматическое удаление стенда не начинается.
